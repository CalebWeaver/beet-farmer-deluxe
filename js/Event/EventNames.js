LEARN_TO_FARM = Event.wrapText("Learn to Farm",
    "Observing the dirt, you notice potential.");

BEETS_BEGIN = Event.wrapText("Beets Begin",
    "The richest of men begin with beets.");

HUNGER_SETS_IN = Event.wrapText("Hunger Sets In",
    "You seem to be hungry for growth.");

OFF_TO_MARKET = Event.wrapText("Off To Market",
    "It seems you've found something to do with these rubies.");

DAY_ON_FARM = Event.wrapText("A Day on the Farm",
    "Farm life isn't the most exciting, but it is rewarding.",
    ["Get extra beets."]);

DAY_ON_FARM_REWARD = Event.wrapText("A Day on the Farm");

FIRST_HARVEST = Event.wrapText("First Harvest",
    "Your first big harvest has come in.",
    ["Replant the roots.",
        "Keep them for your stores."]);

FIRST_HARVEST_PLANT = Event.wrapText("Long-Term Vision",
    "You can stand a little hunger, ready to see the harvest next season.");

FIRST_HARVEST_KEEP = Event.wrapText("Tomorrow is no Guarantee",
    "These gems will be just as useful today as they ever will be.");

KNOWLEDGE_FARMING_DISCOVERY = Event.wrapText("Knowledge of Farming",
    "Keep your head down and eyes on the dirt.");

CROWN_OF_ROOTS = Event.wrapText("Crown of Roots",
    "You find a cluster of small beetroots which seem to have shaped themselves into a ring." +
    " It seems to be roughly the size of your head.",
    ["These beets belong in the ground.",
        "Don the wreath."]);

BEET_KING = Event.wrapText("King of Beets",
    "You've become more understanding of your subjects.");

FARM_SAVANT = Event.wrapText("Farm Savant",
    "Those roots will blossom nicely.");

GAINING_KNOWLEDGE = Event.wrapText("Gaining Knowledge",
    "You've come to understand more of what the ground can accommodate. But should you proceed?",
    ["Explore alternative plants",
    "Remain pure"]);

BEET_PURIST = Event.wrapText("Beet Purist",
    "A wise decision. Your produce will beet all other.");

VEGETAL_EXPLORATION = Event.wrapText("Vegetal Exploration",
    "A bold decision. But will it yield any fruit?");

BAARDVARK_FARM = Event.wrapText("A Curious Minstrel",
    "A fellow with a lute strapped to his back and a long snout-like nose wanders up to your farm. "+
    "He asks if you have some spare food in exchange for some song.",
    ["Offer him some work.",
        "Give him some beets and hear his song. - 250 Beets"]);

BAARDVARK_FARM_FEED = Event.wrapText("A Happy Player",
    "The odd-looking bard thanks you for your generous hospitality and returns the favor with "+
    "a song which has a lot to do with spelling.");

BAARDVARK_FARM_WORK = Event.wrapText("A Willing Worker",
    "The besnouted bard accepts your offer of work and after a long day, takes his share of the profits.");

FARM_REMAINS = Event.wrapText("Skull",
    "Seems you've found a small bone-us while trowling around.",
    ["Leave the remains at peace.",
    "This ground is meant for your reds. Dig them up.",
    "Erect a small monument for the lost. - 30 Gold"]);

LEAVE_THE_BONES = Event.wrapText("A Modicum of Respect",
"Some things are better left untouched.");

DIG_UP_BONES = Event.wrapText("Keep the Soil Pure",
"Only the living matter, and these beets need to grow.");

FARM_MONUMENT = Event.wrapText("A Humble Testament",
"You feel uplifted, but you're not sure why.");

FARM_CHEST = Event.wrapText("Chest",
    "You've found a big, square, wooden beet?",
    ["Open Chest"]);

FARM_CHEST_OPEN = Event.wrapText("Inside the Chest",
    "The chest contains a number of old items, but nothing that speaks of treasure and nothing you need.",
    ["Small Yellow Kernels.",
    "Tarnished Medallion"]);

BEETS_BEYOND = Event.wrapText("Beets Beyond",
    "You've found some curious new seeds at the market.");

CURSE_TEN = Event.wrapText("An Ill Feeling",
"Something strikes you as feeling somehow rwong.");

CURSE_THIRTY = Event.wrapText("More Ominous",
"There seem to be far more black cats around the farm than you previously remember.");

CURSE_SIXTY = Event.wrapText("Whispers in the Night",
"You've started borrowing books to determine what is a sign of bad luck and you've determined all the ducks are not technically one of them.");

CURSE_FINAL = Event.wrapText("Poor Unfortunate Souls",
"You've concluded the universe very much dislikes your very essence.");

CURSE_LIFTING = Event.wrapText("A Brighter Tomorrow",
"The beets seem cheery.");

WEEDS = Event.wrapText("Weeds",
    "You could be wrong, but those don't look like beets to you.");

CENTIPEDES = Event.wrapText("Centipedes",
    "Centipedes have overrun your farm and are now eating your beets. (Pest Control: 10)");

CROWS = Event.wrapText("Crows",
    "A flock of crows has decided to roost in your fields. Seems they're trying to rook you of your beets.");

POSSESSED_SCARECROW = Event.wrapText("Scarecrow",
    "...did that scarecrow just move?");

METEOR = Event.wrapText("Meteor",
    "A large meteor just fell from the sky.");